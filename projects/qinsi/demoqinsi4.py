...
def get_allday(id,now_time,day,num):   #近30天销量数据
    ...
    proxy = {'http':"","https":""}
    response1 = requests.post('https://web.qinsilk.com/is/admin/inner/report/sale/saleGroupBySkuListJSON.ac', headers=headers,cookies=cookies, data=data1,timeout=10, verify=False, proxies=proxy)
    ...
    for page in range(1,pages+1):
        ...
        response = requests.post('https://web.qinsilk.com/is/admin/inner/report/sale/saleGroupBySkuListJSON.ac',
                                  headers=headers,cookies=cookies, data=data, timeout=10, verify=False, proxies=proxy)
        datas = json.loads(response.text)
        print(f'正在获取第{page}页的数据')
        time.sleep(1.25)
        for row in datas["rows"]:
